...
from ui.item import LootListSelectorView
  ...
  async def loot(self, ctx,
    ...
    try:
      ...
      if item_name is not None and item_id is not None:
        raise InvalidArgument("provide either name or _id, not both.")
      ...
      async with self.bot.db_session() as sess:
        async with sess.begin():
          ...
          items = items_search(sess, item_name, item_id)
          view = LootListSelectorView(self, items, character.id)
          ctx.respond(view=view)
